...
impl LinkDrop {
    ...
    pub fn send(&mut self, public_key: PublicKey) -> Promise {
        ...
        println!("Env::Attached Deposit: {}", env::attached_deposit());
    ...
    pub fn list_accounts(&self) -> Vec<(PublicKey, u128)> {
        let keys = self.accounts.to_vec();
